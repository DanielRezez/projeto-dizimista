...
function Pagina_inicial() {
    ...
    const handleComunidadeChange = (e) => {
        ...
        setComunidade(selectedId);
        const selectedParoquia = paroquias.find(p => p.id === selectedId);
        setNomeComunidade(selectedParoquia ? selectedParoquia.nome : '');
    };
        ...
        <Layout>
            <div className="print:block flex justify-center w-full -mt-[80px]">
                <section className="print:mt-10 max-w-[90%] px-30 pb-30 my-[115px] w-[1280px] min-h-[720px] h-auto rounded-sm shadow-[4px_4px_4px_rgba(0,0,0,0.25)] bg-gradient-to-t from-[#EDE8DD] to-[#FFFFFF]">
                    ...
                    {/* Nome da Comunidade para Impressão */}
                    <div className="hidden print:block text-center text-[1.5rem] font-[Tangerine] text-[#C9942B] mt-12 mb-16">
                    ...
                    <div className="print:items-center flex flex-col items-start gap-4 my-[58px] md:flex md:flex-row md:justify-around md:flex-wrap">
                        <select 
                            className="print:text-center bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-amber-400 focus:border-amber-400 block w-full p-2.5 max-w-100" 
                            value={comunidade} 
