    ...
    return (
        <Layout>
            <div className="flex flex-col items-center justify-start min-h-screen w-full px-4">
                ...
                <section className="@container flex flex-col w-full max-w-[1280px] min-h-[60%] h-auto bg-gradient-to-t from-[#EDE8DD] to-[#FFFFFF] shadow-lg rounded-lg p-5">
                    <div className=" overflow-x-auto">
                        <Tabela_dizimistas pagina={paginaAtual} itensPorPagina={itensPorPagina} />   
                    </div>

                    <div className="flex justify-center w-[300px] md:w-5/5 bg-500-200 mt-24">
                        {paginas.map((num) => (
                            <button
                            key={num}
                            onClick={() => setPaginaAtual(num)}
                            className={`mx-1 px-3 py-1 rounded-md ${
                                num === paginaAtual ? "bg-[#A10013] text-white" : "bg-gray-200 text-gray-700"
                            } hover:bg-gray-300`}
                        >
                            {num}
                        </button>   
